...
add_action('plugins_loaded', 'init_shieldclimbcryptogateway_eth_gateway');

function init_shieldclimbcryptogateway_eth_gateway() {
...
protected $icon_url;
    ...
    public function __construct() {
        $this->id                 = 'shieldclimb-eth';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/eth.png');
        $this->method_title       = esc_html__('Ethereum (ERC20)  - ShieldClimb Crypto Payment Gateway (Auto-hide at checkout if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
        $this->method_description = esc_html__('Ethereum (ERC20) crypto payment gateway with instant payouts, no sign-up, no KYC, and on-site checkout, sending funds directly to your eth wallet.', 'shieldclimb-crypto-payment-gateway'); // Escaping description
...
$shieldclimbcryptogateway_eth_response = wp_remote_get('https://api.shieldclimb.com/crypto/eth/convert.php?value=' . $shieldclimbcryptogateway_eth_total . '&from=' . strtolower($shieldclimbcryptogateway_eth_currency), array('timeout' => 30));
...
if ($shieldclimbcryptogateway_eth_conversion_resp && isset($shieldclimbcryptogateway_eth_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$shieldclimbcryptogateway_eth_feesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/eth/aff-fees.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_eth_revfeesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/eth/convert.php?value=' . $shieldclimbcryptogateway_eth_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$shieldclimbcryptogateway_eth_response_minimum = wp_remote_get('https://api.shieldclimb.com/crypto/eth/aff-info.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_eth_gen_wallet = wp_remote_get('https://api.shieldclimb.com/crypto/eth/wallet.php?address=' . $this->eth_wallet_address .'&callback=' . urlencode($shieldclimbcryptogateway_eth_callback), array('timeout' => 30));
...
if (is_wp_error($shieldclimbcryptogateway_eth_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($shieldclimbcryptogateway_eth_wallet_decbody && isset($shieldclimbcryptogateway_eth_wallet_decbody['address_in'])) {
		...
		$shieldclimbcryptogateway_eth_genqrcode_response = wp_remote_get('https://api.shieldclimb.com/crypto/eth/qrcode.php?address=' . $shieldclimbcryptogateway_eth_gen_addressIn, array('timeout' => 30));
...
function shieldclimbcryptogateway_add_instant_payment_gateway_eth($gateways) {
    $gateways[] = 'shieldclimb_Crypto_Payment_Gateway_Eth';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbcryptogateway_add_instant_payment_gateway_eth');
}

// Add custom endpoint for reading crypto payment status

   function shieldclimbcryptogateway_eth_check_order_status_rest_endpoint() {
        register_rest_route('shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-check-order-status-eth/', array(
            'methods'  => 'GET',
    ...
    function shieldclimbcryptogateway_eth_check_order_status_callback($request) {
		...
		$shieldclimbcryptogateway_eth_live_status_nonce = sanitize_text_field($request->get_param('nonce'));
        ...
        if ( empty( $shieldclimbcryptogateway_eth_live_status_nonce ) || $order->get_meta('shieldclimb_eth_status_nonce', true) !== $shieldclimbcryptogateway_eth_live_status_nonce ) {
...
function shieldclimbcryptogateway_eth_change_order_status_rest_endpoint() {
    ...
    register_rest_route( 'shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-eth/', array(
        'methods'  => 'GET',
...
function shieldclimbcryptogateway_eth_change_order_status_callback( $request ) {
    $order_id = absint($request->get_param( 'order_id' ));
	$shieldclimbcryptogateway_ethgetnonce = sanitize_text_field($request->get_param( 'nonce' ));
	$shieldclimbcryptogateway_ethpaid_value_coin = sanitize_text_field($request->get_param('value_coin'));
	$shieldclimbcryptogateway_eth_paid_coin_name = sanitize_text_field($request->get_param('coin'));
	$shieldclimbcryptogateway_eth_paid_txid_in = sanitize_text_field($request->get_param('txid_in'));
	...
	// Verify nonce
    if ( empty( $shieldclimbcryptogateway_ethgetnonce ) || $order->get_meta('shieldclimb_eth_nonce', true) !== $shieldclimbcryptogateway_ethgetnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-crypto-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-eth'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-eth' === $order->get_payment_method() ) {
		
		// Get the expected amount and coin
	$shieldclimbcryptogateway_ethexpected_amount = $order->get_meta('shieldclimb_eth_payin_amount', true) * $order->get_meta('shieldclimb_eth_tolerance_percentage', true);

	
		if ( $shieldclimbcryptogateway_ethpaid_value_coin < $shieldclimbcryptogateway_ethexpected_amount || $shieldclimbcryptogateway_eth_paid_coin_name !== 'eth') {
			// Mark the order as failed and add an order note
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */			
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s eth. TXID: %4$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_ethpaid_value_coin, $shieldclimbcryptogateway_eth_paid_coin_name, $shieldclimbcryptogateway_ethexpected_amount, $shieldclimbcryptogateway_eth_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
...
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_ethpaid_value_coin, $shieldclimbcryptogateway_eth_paid_coin_name, $shieldclimbcryptogateway_eth_paid_txid_in));
