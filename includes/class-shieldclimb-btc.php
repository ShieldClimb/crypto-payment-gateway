...
add_action('plugins_loaded', 'init_shieldclimbcryptogateway_btc_gateway');

function init_shieldclimbcryptogateway_btc_gateway() {
...
protected $icon_url;
    ...
    public function __construct() {
        $this->id                 = 'shieldclimb-btc';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/btc.png');
        $this->method_title       = esc_html__('Bitcoin  - ShieldClimb Crypto Payment Gateway (Auto-hide at checkout if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
        $this->method_description = esc_html__('Bitcoin crypto payment gateway with instant payouts, no sign-up, no KYC, and on-site checkout, sending funds directly to your btc wallet.', 'shieldclimb-crypto-payment-gateway'); // Escaping description
...
$shieldclimbcryptogateway_btc_response = wp_remote_get('https://api.shieldclimb.com/crypto/btc/convert.php?value=' . $shieldclimbcryptogateway_btc_total . '&from=' . strtolower($shieldclimbcryptogateway_btc_currency), array('timeout' => 30));
...
if ($shieldclimbcryptogateway_btc_conversion_resp && isset($shieldclimbcryptogateway_btc_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$shieldclimbcryptogateway_btc_feesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/btc/aff-fees.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_btc_revfeesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/btc/convert.php?value=' . $shieldclimbcryptogateway_btc_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$shieldclimbcryptogateway_btc_response_minimum = wp_remote_get('https://api.shieldclimb.com/crypto/btc/aff-info.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_btc_gen_wallet = wp_remote_get('https://api.shieldclimb.com/crypto/btc/wallet.php?address=' . $this->btc_wallet_address .'&callback=' . urlencode($shieldclimbcryptogateway_btc_callback), array('timeout' => 30));
...
if (is_wp_error($shieldclimbcryptogateway_btc_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($shieldclimbcryptogateway_btc_wallet_decbody && isset($shieldclimbcryptogateway_btc_wallet_decbody['address_in'])) {
		...
		$shieldclimbcryptogateway_btc_genqrcode_response = wp_remote_get('https://api.shieldclimb.com/crypto/btc/qrcode.php?address=' . $shieldclimbcryptogateway_btc_gen_addressIn, array('timeout' => 30));
...
function shieldclimbcryptogateway_add_instant_payment_gateway_btc($gateways) {
    $gateways[] = 'shieldclimb_Crypto_Payment_Gateway_Btc';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbcryptogateway_add_instant_payment_gateway_btc');
}

// Add custom endpoint for reading crypto payment status

   function shieldclimbcryptogateway_btc_check_order_status_rest_endpoint() {
        register_rest_route('shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-check-order-status-btc/', array(
            'methods'  => 'GET',
    ...
    function shieldclimbcryptogateway_btc_check_order_status_callback($request) {
		...
		$shieldclimbcryptogateway_btc_live_status_nonce = sanitize_text_field($request->get_param('nonce'));
        ...
        if ( empty( $shieldclimbcryptogateway_btc_live_status_nonce ) || $order->get_meta('shieldclimb_btc_status_nonce', true) !== $shieldclimbcryptogateway_btc_live_status_nonce ) {
...
function shieldclimbcryptogateway_btc_change_order_status_rest_endpoint() {
    ...
    register_rest_route( 'shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-btc/', array(
        'methods'  => 'GET',
...
function shieldclimbcryptogateway_btc_change_order_status_callback( $request ) {
    $order_id = absint($request->get_param( 'order_id' ));
	$shieldclimbcryptogateway_btcgetnonce = sanitize_text_field($request->get_param( 'nonce' ));
	$shieldclimbcryptogateway_btcpaid_value_coin = sanitize_text_field($request->get_param('value_coin'));
	$shieldclimbcryptogateway_btc_paid_coin_name = sanitize_text_field($request->get_param('coin'));
	$shieldclimbcryptogateway_btc_paid_txid_in = sanitize_text_field($request->get_param('txid_in'));
	...
	// Verify nonce
    if ( empty( $shieldclimbcryptogateway_btcgetnonce ) || $order->get_meta('shieldclimb_btc_nonce', true) !== $shieldclimbcryptogateway_btcgetnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-crypto-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-btc'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-btc' === $order->get_payment_method() ) {
		
		// Get the expected amount and coin
	$shieldclimbcryptogateway_btcexpected_amount = $order->get_meta('shieldclimb_btc_payin_amount', true) * $order->get_meta('shieldclimb_btc_tolerance_percentage', true);

	
		if ( $shieldclimbcryptogateway_btcpaid_value_coin < $shieldclimbcryptogateway_btcexpected_amount || $shieldclimbcryptogateway_btc_paid_coin_name !== 'btc') {
			// Mark the order as failed and add an order note
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */			
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s btc. TXID: %4$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_btcpaid_value_coin, $shieldclimbcryptogateway_btc_paid_coin_name, $shieldclimbcryptogateway_btcexpected_amount, $shieldclimbcryptogateway_btc_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
...
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_btcpaid_value_coin, $shieldclimbcryptogateway_btc_paid_coin_name, $shieldclimbcryptogateway_btc_paid_txid_in));
