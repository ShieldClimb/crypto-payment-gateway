...
function shieldclimbcryptogateway_btc_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'shieldclimb-crypto-payment-gateway-btc'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-crypto-payment-gateway-btc' === $order->get_payment_method() ) {
