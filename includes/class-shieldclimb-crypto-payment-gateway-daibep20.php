...
protected $icon_url;
    ...
    public function __construct() {
        ...
        $this->method_title       = esc_html__('ShieldClimb – Crypto Payment Gateway (Dai Token bep20 | Min 1.33dai | Auto-hide if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
...
function shieldclimbcryptogateway_daibep20_check_order_status_rest_endpoint() {
    register_rest_route(
        ...
        '/shieldclimbcryptogateway-check-order-status-daibep20/', 
        array(
            'methods'  => 'GET',
            'callback' => 'shieldclimbcryptogateway_daibep20_check_order_status_callback',
            'permission_callback' => 'shieldclimbcryptogateway_daibep20_check_status_permission'
        )
    );
}
...
function shieldclimbcryptogateway_daibep20_check_status_permission($request) {
...
function shieldclimbcryptogateway_daibep20_change_order_status_rest_endpoint() {
    register_rest_route(
        'shieldclimbcryptogateway/v1',
        '/shieldclimbcryptogateway-daibep20/',
        array(
            'methods'  => 'GET',
            'callback' => 'shieldclimbcryptogateway_daibep20_change_order_status_callback',
            'permission_callback' => 'shieldclimbcryptogateway_daibep20_change_status_permission'
        )
    );
}
...
function shieldclimbcryptogateway_daibep20_change_status_permission($request) {
    $order_id = absint($request->get_param('order_id'));
    $nonce = sanitize_text_field($request->get_param('nonce'));
...
function shieldclimbcryptogateway_daibep20_change_order_status_callback($request) {
    ...
    if (!in_array($order->get_status(), ['processing', 'completed'], true) 
        ...
        $expected_amount = (float)$order->get_meta('shieldclimb_daibep20_payin_amount', true)
