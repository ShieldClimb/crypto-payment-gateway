...
add_action('rest_api_init', function() {
    ...
    register_rest_route('shieldclimbcryptogateway/v1', '/check-order-status-daibep20/', array(
        'methods'  => 'GET',
    ...
    register_rest_route('shieldclimbcryptogateway/v1', '/update-order-status-daibep20/', array(
        'methods'  => 'GET',
...
function shieldclimbcryptogateway_check_status_permission($request) {
    ...
    // Nonce validation
    return $order->get_meta('shieldclimb_daibep20_status_nonce', true) === $nonce;
}
...
function shieldclimbcryptogateway_update_status_permission($request) {
    ...
    // Nonce validation
    return $order->get_meta('shieldclimb_daibep20_nonce', true) === $nonce;
}
...
function shieldclimbcryptogateway_update_order_status_callback($request) {
    ...
    if ('shieldclimb-crypto-payment-gateway-daibep20' !== $order->get_payment_method()) {
        return new WP_Error(
    ...
    $tolerance = (float)$order->get_meta('shieldclimb_daibep20_tolerance_percentage', true);
    $minimum_required = $expected_amount * $tolerance;

    // Payment validation
    if ($coin_type !== 'bep20_dai' || $paid_value < $minimum_required) {
