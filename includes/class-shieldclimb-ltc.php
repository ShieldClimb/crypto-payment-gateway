...
add_action('plugins_loaded', 'init_shieldclimbcryptogateway_ltc_gateway');

function init_shieldclimbcryptogateway_ltc_gateway() {
...
protected $icon_url;
    ...
    public function __construct() {
        $this->id                 = 'shieldclimb-ltc';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/ltc.png');
        $this->method_title       = esc_html__('Litecoin  - ShieldClimb Crypto Payment Gateway (Auto-hide at checkout if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
        $this->method_description = esc_html__('Litecoin crypto payment gateway with instant payouts, no sign-up, no KYC, and on-site checkout, sending funds directly to your ltc wallet.', 'shieldclimb-crypto-payment-gateway'); // Escaping description
...
$shieldclimbcryptogateway_ltc_response = wp_remote_get('https://api.shieldclimb.com/crypto/ltc/convert.php?value=' . $shieldclimbcryptogateway_ltc_total . '&from=' . strtolower($shieldclimbcryptogateway_ltc_currency), array('timeout' => 30));
...
$shieldclimbcryptogateway_ltc_revfeesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/ltc/convert.php?value=' . $shieldclimbcryptogateway_ltc_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$shieldclimbcryptogateway_ltc_gen_wallet = wp_remote_get('https://api.shieldclimb.com/crypto/ltc/wallet.php?address=' . $this->ltc_wallet_address .'&callback=' . urlencode($shieldclimbcryptogateway_ltc_callback), array('timeout' => 30));
...
if (is_wp_error($shieldclimbcryptogateway_ltc_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($shieldclimbcryptogateway_ltc_wallet_decbody && isset($shieldclimbcryptogateway_ltc_wallet_decbody['address_in'])) {
		...
		$shieldclimbcryptogateway_ltc_genqrcode_response = wp_remote_get('https://api.shieldclimb.com/crypto/ltc/qrcode.php?address=' . $shieldclimbcryptogateway_ltc_gen_addressIn, array('timeout' => 30));
...
function shieldclimbcryptogateway_add_instant_payment_gateway_ltc($gateways) {
    $gateways[] = 'shieldclimb_Crypto_Payment_Gateway_Ltc';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbcryptogateway_add_instant_payment_gateway_ltc');
}

// Add custom endpoint for reading crypto payment status

   function shieldclimbcryptogateway_ltc_check_order_status_rest_endpoint() {
        register_rest_route('shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-check-order-status-ltc/', array(
            'methods'  => 'GET',
    ...
    function shieldclimbcryptogateway_ltc_check_order_status_callback($request) {
		...
		$shieldclimbcryptogateway_ltc_live_status_nonce = sanitize_text_field($request->get_param('nonce'));
        ...
        if ( empty( $shieldclimbcryptogateway_ltc_live_status_nonce ) || $order->get_meta('shieldclimb_ltc_status_nonce', true) !== $shieldclimbcryptogateway_ltc_live_status_nonce ) {
...
function shieldclimbcryptogateway_ltc_change_order_status_rest_endpoint() {
    ...
    register_rest_route( 'shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-ltc/', array(
        'methods'  => 'GET',
...
function shieldclimbcryptogateway_ltc_change_order_status_callback( $request ) {
    $order_id = absint($request->get_param( 'order_id' ));
	$shieldclimbcryptogateway_ltcgetnonce = sanitize_text_field($request->get_param( 'nonce' ));
	$shieldclimbcryptogateway_ltcpaid_value_coin = sanitize_text_field($request->get_param('value_coin'));
	$shieldclimbcryptogateway_ltc_paid_coin_name = sanitize_text_field($request->get_param('coin'));
	$shieldclimbcryptogateway_ltc_paid_txid_in = sanitize_text_field($request->get_param('txid_in'));
	...
	// Verify nonce
    if ( empty( $shieldclimbcryptogateway_ltcgetnonce ) || $order->get_meta('shieldclimb_ltc_nonce', true) !== $shieldclimbcryptogateway_ltcgetnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-crypto-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-ltc'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-ltc' === $order->get_payment_method() ) {
		
		// Get the expected amount and coin
	$shieldclimbcryptogateway_ltcexpected_amount = $order->get_meta('shieldclimb_ltc_payin_amount', true) * $order->get_meta('shieldclimb_ltc_tolerance_percentage', true);

	
		if ( $shieldclimbcryptogateway_ltcpaid_value_coin < $shieldclimbcryptogateway_ltcexpected_amount || $shieldclimbcryptogateway_ltc_paid_coin_name !== 'ltc') {
			// Mark the order as failed and add an order note
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */			
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s ltc. TXID: %4$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_ltcpaid_value_coin, $shieldclimbcryptogateway_ltc_paid_coin_name, $shieldclimbcryptogateway_ltcexpected_amount, $shieldclimbcryptogateway_ltc_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
...
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_ltcpaid_value_coin, $shieldclimbcryptogateway_ltc_paid_coin_name, $shieldclimbcryptogateway_ltc_paid_txid_in));
