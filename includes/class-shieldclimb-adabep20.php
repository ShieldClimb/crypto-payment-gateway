...
add_action('plugins_loaded', 'init_shieldclimbcryptogateway_adabep20_gateway');

function init_shieldclimbcryptogateway_adabep20_gateway() {
...
protected $icon_url;
    ...
    public function __construct() {
        $this->id                 = 'shieldclimb-adabep20';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/adabep20.png');
        $this->method_title       = esc_html__('Cardano bep20  - ShieldClimb Crypto Payment Gateway (Auto-hide at checkout if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
        $this->method_description = esc_html__('Cardano bep20 crypto payment gateway with instant payouts, no sign-up, no KYC, and on-site checkout, sending funds directly to your bep20_ada wallet.', 'shieldclimb-crypto-payment-gateway'); // Escaping description
...
$shieldclimbcryptogateway_adabep20_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/ada/convert.php?value=' . $shieldclimbcryptogateway_adabep20_total . '&from=' . strtolower($shieldclimbcryptogateway_adabep20_currency), array('timeout' => 30));
...
if ($shieldclimbcryptogateway_adabep20_conversion_resp && isset($shieldclimbcryptogateway_adabep20_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$shieldclimbcryptogateway_adabep20_feesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/ada/aff-fees.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_adabep20_revfeesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/ada/convert.php?value=' . $shieldclimbcryptogateway_adabep20_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$shieldclimbcryptogateway_adabep20_response_minimum = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/ada/aff-info.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_adabep20_gen_wallet = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/ada/wallet.php?address=' . $this->adabep20_wallet_address .'&callback=' . urlencode($shieldclimbcryptogateway_adabep20_callback), array('timeout' => 30));
...
if (is_wp_error($shieldclimbcryptogateway_adabep20_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($shieldclimbcryptogateway_adabep20_wallet_decbody && isset($shieldclimbcryptogateway_adabep20_wallet_decbody['address_in'])) {
		...
		$shieldclimbcryptogateway_adabep20_genqrcode_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/ada/qrcode.php?address=' . $shieldclimbcryptogateway_adabep20_gen_addressIn, array('timeout' => 30));
...
function shieldclimbcryptogateway_add_instant_payment_gateway_adabep20($gateways) {
    $gateways[] = 'shieldclimb_Crypto_Payment_Gateway_Adabep20';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbcryptogateway_add_instant_payment_gateway_adabep20');
}

// Add custom endpoint for reading crypto payment status

   function shieldclimbcryptogateway_adabep20_check_order_status_rest_endpoint() {
        register_rest_route('shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-check-order-status-adabep20/', array(
            'methods'  => 'GET',
            'callback' => 'shieldclimbcryptogateway_adabep20_check_order_status_callback',
            'permission_callback' => '__return_true',
        ));
    }

    add_action('rest_api_init', 'shieldclimbcryptogateway_adabep20_check_order_status_rest_endpoint');

    function shieldclimbcryptogateway_adabep20_check_order_status_callback($request) {
        $order_id = absint($request->get_param('order_id'));
		$shieldclimbcryptogateway_adabep20_live_status_nonce = sanitize_text_field($request->get_param('nonce'));
        ...
        if ( empty( $shieldclimbcryptogateway_adabep20_live_status_nonce ) || $order->get_meta('shieldclimb_adabep20_status_nonce', true) !== $shieldclimbcryptogateway_adabep20_live_status_nonce ) {
...
function shieldclimbcryptogateway_adabep20_change_order_status_rest_endpoint() {
    // Register custom route
    register_rest_route( 'shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-adabep20/', array(
        'methods'  => 'GET',
        'callback' => 'shieldclimbcryptogateway_adabep20_change_order_status_callback',
        'permission_callback' => '__return_true',
    ));
}
add_action( 'rest_api_init', 'shieldclimbcryptogateway_adabep20_change_order_status_rest_endpoint' );

// Callback function to change order status
function shieldclimbcryptogateway_adabep20_change_order_status_callback( $request ) {
    $order_id = absint($request->get_param( 'order_id' ));
	$shieldclimbcryptogateway_adabep20getnonce = sanitize_text_field($request->get_param( 'nonce' ));
	$shieldclimbcryptogateway_adabep20paid_value_coin = sanitize_text_field($request->get_param('value_coin'));
	$shieldclimbcryptogateway_adabep20_paid_coin_name = sanitize_text_field($request->get_param('coin'));
	$shieldclimbcryptogateway_adabep20_paid_txid_in = sanitize_text_field($request->get_param('txid_in'));
	...
	// Verify nonce
    if ( empty( $shieldclimbcryptogateway_adabep20getnonce ) || $order->get_meta('shieldclimb_adabep20_nonce', true) !== $shieldclimbcryptogateway_adabep20getnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-crypto-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-adabep20'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-adabep20' === $order->get_payment_method() ) {
		
		// Get the expected amount and coin
	$shieldclimbcryptogateway_adabep20expected_amount = $order->get_meta('shieldclimb_adabep20_payin_amount', true) * $order->get_meta('shieldclimb_adabep20_tolerance_percentage', true);

	
		if ( $shieldclimbcryptogateway_adabep20paid_value_coin < $shieldclimbcryptogateway_adabep20expected_amount || $shieldclimbcryptogateway_adabep20_paid_coin_name !== 'bep20_ada') {
			// Mark the order as failed and add an order note
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */			
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s bep20_ada. TXID: %4$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_adabep20paid_value_coin, $shieldclimbcryptogateway_adabep20_paid_coin_name, $shieldclimbcryptogateway_adabep20expected_amount, $shieldclimbcryptogateway_adabep20_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
...
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_adabep20paid_value_coin, $shieldclimbcryptogateway_adabep20_paid_coin_name, $shieldclimbcryptogateway_adabep20_paid_txid_in));
