...
add_action('plugins_loaded', 'shieldclimbcryptogateway_usdtbep20_gateway');

function shieldclimbcryptogateway_usdtbep20_gateway() {
...
protected $icon_url;
    ...
    public function __construct() {
        ...
        $this->method_title       = esc_html__('ShieldClimb – Crypto Payment Gateway (USDT bep20 | Min 1.33usdt | Auto-hide if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
...
function shieldclimbcryptogateway_usdtbep20_check_order_status_rest_endpoint() {
    register_rest_route(
        ...
        '/shieldclimbcryptogateway-check-order-status-usdtbep20/', 
        array(
            'methods'  => 'GET',
            'callback' => 'shieldclimbcryptogateway_usdtbep20_check_order_status_callback',
            'permission_callback' => 'shieldclimbcryptogateway_usdtbep20_check_status_permission'
        )
    );
}
...
function shieldclimbcryptogateway_usdtbep20_check_status_permission($request) {
    ...
    if ($order->get_meta('shieldclimb_usdtbep20_status_nonce', true) !== $nonce) {
        return new WP_Error(
            'rest_forbidden',
...
function shieldclimbcryptogateway_usdtbep20_change_order_status_rest_endpoint() {
    register_rest_route(
        'shieldclimbcryptogateway/v1',
        '/shieldclimbcryptogateway-usdtbep20/',
        array(
            'methods'  => 'GET',
            'callback' => 'shieldclimbcryptogateway_usdtbep20_change_order_status_callback',
            'permission_callback' => 'shieldclimbcryptogateway_usdtbep20_change_status_permission'
        )
    );
}
...
function shieldclimbcryptogateway_usdtbep20_change_status_permission($request) {
    $order_id = absint($request->get_param('order_id'));
    $nonce = sanitize_text_field($request->get_param('nonce'));
    ...
    if ($order->get_meta('shieldclimb_usdtbep20_nonce', true) !== $nonce) {
        return new WP_Error(
            'rest_forbidden',
            __('Invalid nonce.', 'shieldclimb-crypto-payment-gateway'),
            array('status' => 403)
...
function shieldclimbcryptogateway_usdtbep20_change_order_status_callback($request) {
    ...
    if (!in_array($order->get_status(), ['processing', 'completed'], true) 
        && 'shieldclimb-crypto-payment-gateway-usdtbep20' === $order->get_payment_method()
    ) {
        $expected_amount = (float)$order->get_meta('shieldclimb_usdtbep20_payin_amount', true)
                         * (float)$order->get_meta('shieldclimb_usdtbep20_tolerance_percentage', true);

        if ($paid_value_coin < $expected_amount || $paid_coin_name !== 'bep20_usdt') {
            $note = sprintf(
                /* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
                __('[Order Failed] Customer sent %1$s %2$s instead of %3$s bep20_usdt. TXID: %4$s', 'shieldclimb-crypto-payment-gateway'),
                $paid_value_coin,
                $paid_coin_name,
                $expected_amount,
                $paid_txid_in
