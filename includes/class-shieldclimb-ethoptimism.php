...
add_action('plugins_loaded', 'init_shieldclimbcryptogateway_ethoptimism_gateway');

function init_shieldclimbcryptogateway_ethoptimism_gateway() {
...
protected $icon_url;
    ...
    public function __construct() {
        $this->id                 = 'shieldclimb-ethoptimism';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/ethoptimism.png');
        $this->method_title       = esc_html__('Ethereum optimism  - ShieldClimb Crypto Payment Gateway (Auto-hide at checkout if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
        $this->method_description = esc_html__('Ethereum optimism crypto payment gateway with instant payouts, no sign-up, no KYC, and on-site checkout, sending funds directly to your optimism_eth wallet.', 'shieldclimb-crypto-payment-gateway'); // Escaping description
...
$shieldclimbcryptogateway_ethoptimism_response = wp_remote_get('https://api.shieldclimb.com/crypto/optimism/eth/convert.php?value=' . $shieldclimbcryptogateway_ethoptimism_total . '&from=' . strtolower($shieldclimbcryptogateway_ethoptimism_currency), array('timeout' => 30));
...
if ($shieldclimbcryptogateway_ethoptimism_conversion_resp && isset($shieldclimbcryptogateway_ethoptimism_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$shieldclimbcryptogateway_ethoptimism_feesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/optimism/eth/aff-fees.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_ethoptimism_revfeesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/optimism/eth/convert.php?value=' . $shieldclimbcryptogateway_ethoptimism_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$shieldclimbcryptogateway_ethoptimism_response_minimum = wp_remote_get('https://api.shieldclimb.com/crypto/optimism/eth/aff-info.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_ethoptimism_gen_wallet = wp_remote_get('https://api.shieldclimb.com/crypto/optimism/eth/wallet.php?address=' . $this->ethoptimism_wallet_address .'&callback=' . urlencode($shieldclimbcryptogateway_ethoptimism_callback), array('timeout' => 30));
...
if (is_wp_error($shieldclimbcryptogateway_ethoptimism_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($shieldclimbcryptogateway_ethoptimism_wallet_decbody && isset($shieldclimbcryptogateway_ethoptimism_wallet_decbody['address_in'])) {
		...
		$shieldclimbcryptogateway_ethoptimism_genqrcode_response = wp_remote_get('https://api.shieldclimb.com/crypto/optimism/eth/qrcode.php?address=' . $shieldclimbcryptogateway_ethoptimism_gen_addressIn, array('timeout' => 30));
...
function shieldclimbcryptogateway_add_instant_payment_gateway_ethoptimism($gateways) {
    $gateways[] = 'shieldclimb_Crypto_Payment_Gateway_Ethoptimism';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbcryptogateway_add_instant_payment_gateway_ethoptimism');
}

// Add custom endpoint for reading crypto payment status

   function shieldclimbcryptogateway_ethoptimism_check_order_status_rest_endpoint() {
        register_rest_route('shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-check-order-status-ethoptimism/', array(
            'methods'  => 'GET',
            'callback' => 'shieldclimbcryptogateway_ethoptimism_check_order_status_callback',
            'permission_callback' => '__return_true',
        ));
    }

    add_action('rest_api_init', 'shieldclimbcryptogateway_ethoptimism_check_order_status_rest_endpoint');

    function shieldclimbcryptogateway_ethoptimism_check_order_status_callback($request) {
        $order_id = absint($request->get_param('order_id'));
		$shieldclimbcryptogateway_ethoptimism_live_status_nonce = sanitize_text_field($request->get_param('nonce'));
        ...
        if ( empty( $shieldclimbcryptogateway_ethoptimism_live_status_nonce ) || $order->get_meta('shieldclimb_ethoptimism_status_nonce', true) !== $shieldclimbcryptogateway_ethoptimism_live_status_nonce ) {
...
function shieldclimbcryptogateway_ethoptimism_change_order_status_rest_endpoint() {
    // Register custom route
    register_rest_route( 'shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-ethoptimism/', array(
        'methods'  => 'GET',
        'callback' => 'shieldclimbcryptogateway_ethoptimism_change_order_status_callback',
        'permission_callback' => '__return_true',
    ));
}
add_action( 'rest_api_init', 'shieldclimbcryptogateway_ethoptimism_change_order_status_rest_endpoint' );

// Callback function to change order status
function shieldclimbcryptogateway_ethoptimism_change_order_status_callback( $request ) {
    $order_id = absint($request->get_param( 'order_id' ));
	$shieldclimbcryptogateway_ethoptimismgetnonce = sanitize_text_field($request->get_param( 'nonce' ));
	$shieldclimbcryptogateway_ethoptimismpaid_value_coin = sanitize_text_field($request->get_param('value_coin'));
	$shieldclimbcryptogateway_ethoptimism_paid_coin_name = sanitize_text_field($request->get_param('coin'));
	$shieldclimbcryptogateway_ethoptimism_paid_txid_in = sanitize_text_field($request->get_param('txid_in'));
	...
	// Verify nonce
    if ( empty( $shieldclimbcryptogateway_ethoptimismgetnonce ) || $order->get_meta('shieldclimb_ethoptimism_nonce', true) !== $shieldclimbcryptogateway_ethoptimismgetnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-crypto-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-ethoptimism'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-ethoptimism' === $order->get_payment_method() ) {
		
		// Get the expected amount and coin
	$shieldclimbcryptogateway_ethoptimismexpected_amount = $order->get_meta('shieldclimb_ethoptimism_payin_amount', true) * $order->get_meta('shieldclimb_ethoptimism_tolerance_percentage', true);

	
		if ( $shieldclimbcryptogateway_ethoptimismpaid_value_coin < $shieldclimbcryptogateway_ethoptimismexpected_amount || $shieldclimbcryptogateway_ethoptimism_paid_coin_name !== 'optimism_eth') {
			// Mark the order as failed and add an order note
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */			
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s optimism_eth. TXID: %4$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_ethoptimismpaid_value_coin, $shieldclimbcryptogateway_ethoptimism_paid_coin_name, $shieldclimbcryptogateway_ethoptimismexpected_amount, $shieldclimbcryptogateway_ethoptimism_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
...
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_ethoptimismpaid_value_coin, $shieldclimbcryptogateway_ethoptimism_paid_coin_name, $shieldclimbcryptogateway_ethoptimism_paid_txid_in));
