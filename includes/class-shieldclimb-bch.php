...
add_action('plugins_loaded', 'init_shieldclimbcryptogateway_bch_gateway');

function init_shieldclimbcryptogateway_bch_gateway() {
...
protected $icon_url;
    ...
    public function __construct() {
        $this->id                 = 'shieldclimb-bch';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/bch.png');
        $this->method_title       = esc_html__('Bitcoin Cash  - ShieldClimb Crypto Payment Gateway (Auto-hide at checkout if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
        $this->method_description = esc_html__('Bitcoin Cash crypto payment gateway with instant payouts, no sign-up, no KYC, and on-site checkout, sending funds directly to your bch wallet.', 'shieldclimb-crypto-payment-gateway'); // Escaping description
...
$shieldclimbcryptogateway_bch_response = wp_remote_get('https://api.shieldclimb.com/crypto/bch/convert.php?value=' . $shieldclimbcryptogateway_bch_total . '&from=' . strtolower($shieldclimbcryptogateway_bch_currency), array('timeout' => 30));
...
if ($shieldclimbcryptogateway_bch_conversion_resp && isset($shieldclimbcryptogateway_bch_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$shieldclimbcryptogateway_bch_feesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/bch/aff-fees.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_bch_revfeesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/bch/convert.php?value=' . $shieldclimbcryptogateway_bch_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$shieldclimbcryptogateway_bch_response_minimum = wp_remote_get('https://api.shieldclimb.com/crypto/bch/aff-info.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_bch_gen_wallet = wp_remote_get('https://api.shieldclimb.com/crypto/bch/wallet.php?address=' . $this->bch_wallet_address .'&callback=' . urlencode($shieldclimbcryptogateway_bch_callback), array('timeout' => 30));
...
if (is_wp_error($shieldclimbcryptogateway_bch_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($shieldclimbcryptogateway_bch_wallet_decbody && isset($shieldclimbcryptogateway_bch_wallet_decbody['address_in'])) {
		...
		$shieldclimbcryptogateway_bch_genqrcode_response = wp_remote_get('https://api.shieldclimb.com/crypto/bch/qrcode.php?address=' . $shieldclimbcryptogateway_bch_gen_addressIn, array('timeout' => 30));
...
function shieldclimbcryptogateway_add_instant_payment_gateway_bch($gateways) {
    $gateways[] = 'shieldclimb_Crypto_Payment_Gateway_Bch';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbcryptogateway_add_instant_payment_gateway_bch');
}

// Add custom endpoint for reading crypto payment status

   function shieldclimbcryptogateway_bch_check_order_status_rest_endpoint() {
        register_rest_route('shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-check-order-status-bch/', array(
            'methods'  => 'GET',
    ...
    function shieldclimbcryptogateway_bch_check_order_status_callback($request) {
		...
		$shieldclimbcryptogateway_bch_live_status_nonce = sanitize_text_field($request->get_param('nonce'));
        ...
        if ( empty( $shieldclimbcryptogateway_bch_live_status_nonce ) || $order->get_meta('shieldclimb_bch_status_nonce', true) !== $shieldclimbcryptogateway_bch_live_status_nonce ) {
...
function shieldclimbcryptogateway_bch_change_order_status_rest_endpoint() {
    ...
    register_rest_route( 'shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-bch/', array(
        'methods'  => 'GET',
...
function shieldclimbcryptogateway_bch_change_order_status_callback( $request ) {
    $order_id = absint($request->get_param( 'order_id' ));
	$shieldclimbcryptogateway_bchgetnonce = sanitize_text_field($request->get_param( 'nonce' ));
	$shieldclimbcryptogateway_bchpaid_value_coin = sanitize_text_field($request->get_param('value_coin'));
	$shieldclimbcryptogateway_bch_paid_coin_name = sanitize_text_field($request->get_param('coin'));
	$shieldclimbcryptogateway_bch_paid_txid_in = sanitize_text_field($request->get_param('txid_in'));
	...
	// Verify nonce
    if ( empty( $shieldclimbcryptogateway_bchgetnonce ) || $order->get_meta('shieldclimb_bch_nonce', true) !== $shieldclimbcryptogateway_bchgetnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-crypto-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-bch'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-bch' === $order->get_payment_method() ) {
		
		// Get the expected amount and coin
	$shieldclimbcryptogateway_bchexpected_amount = $order->get_meta('shieldclimb_bch_payin_amount', true) * $order->get_meta('shieldclimb_bch_tolerance_percentage', true);

	
		if ( $shieldclimbcryptogateway_bchpaid_value_coin < $shieldclimbcryptogateway_bchexpected_amount || $shieldclimbcryptogateway_bch_paid_coin_name !== 'bch') {
			// Mark the order as failed and add an order note
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */			
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s bch. TXID: %4$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_bchpaid_value_coin, $shieldclimbcryptogateway_bch_paid_coin_name, $shieldclimbcryptogateway_bchexpected_amount, $shieldclimbcryptogateway_bch_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
...
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_bchpaid_value_coin, $shieldclimbcryptogateway_bch_paid_coin_name, $shieldclimbcryptogateway_bch_paid_txid_in));
