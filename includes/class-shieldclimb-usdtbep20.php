...
protected $icon_url;
    ...
    public function __construct() {
        $this->id                 = 'shieldclimb-usdtbep20';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/usdtbep20.png');
        $this->method_title       = esc_html__('USDT bep20  - ShieldClimb Crypto Payment Gateway (Auto-hide at checkout if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
        $this->method_description = esc_html__('USDT bep20 crypto payment gateway with instant payouts, no sign-up, no KYC, and on-site checkout, sending funds directly to your bep20_usdt wallet.', 'shieldclimb-crypto-payment-gateway'); // Escaping description
...
$shieldclimbcryptogateway_usdtbep20_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/usdt/convert.php?value=' . $shieldclimbcryptogateway_usdtbep20_total . '&from=' . strtolower($shieldclimbcryptogateway_usdtbep20_currency), array('timeout' => 30));
...
if ($shieldclimbcryptogateway_usdtbep20_conversion_resp && isset($shieldclimbcryptogateway_usdtbep20_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$shieldclimbcryptogateway_usdtbep20_feesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/usdt/aff-fees.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_usdtbep20_revfeesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/usdt/convert.php?value=' . $shieldclimbcryptogateway_usdtbep20_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$shieldclimbcryptogateway_usdtbep20_response_minimum = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/usdt/aff-info.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_usdtbep20_gen_wallet = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/usdt/wallet.php?address=' . $this->usdtbep20_wallet_address .'&callback=' . urlencode($shieldclimbcryptogateway_usdtbep20_callback), array('timeout' => 30));
...
if (is_wp_error($shieldclimbcryptogateway_usdtbep20_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($shieldclimbcryptogateway_usdtbep20_wallet_decbody && isset($shieldclimbcryptogateway_usdtbep20_wallet_decbody['address_in'])) {
		...
		$shieldclimbcryptogateway_usdtbep20_genqrcode_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/usdt/qrcode.php?address=' . $shieldclimbcryptogateway_usdtbep20_gen_addressIn, array('timeout' => 30));
...
function shieldclimbcryptogateway_add_instant_payment_gateway_usdtbep20($gateways) {
    $gateways[] = 'shieldclimb_Crypto_Payment_Gateway_Usdtbep20';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbcryptogateway_add_instant_payment_gateway_usdtbep20');
}
...
function shieldclimbcryptogateway_usdtbep20_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'shieldclimb-usdtbep20'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-usdtbep20' === $order->get_payment_method() ) {
