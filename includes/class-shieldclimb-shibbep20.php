...
add_action('plugins_loaded', 'init_shieldclimbcryptogateway_shibbep20_gateway');

function init_shieldclimbcryptogateway_shibbep20_gateway() {
...
protected $icon_url;
    ...
    public function __construct() {
        $this->id                 = 'shieldclimb-shibbep20';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/shibbep20.png');
        $this->method_title       = esc_html__('SHIBA INU bep20  - ShieldClimb Crypto Payment Gateway (Auto-hide at checkout if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
        $this->method_description = esc_html__('SHIBA INU bep20 crypto payment gateway with instant payouts, no sign-up, no KYC, and on-site checkout, sending funds directly to your bep20_shib wallet.', 'shieldclimb-crypto-payment-gateway'); // Escaping description
...
$shieldclimbcryptogateway_shibbep20_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/shib/convert.php?value=' . $shieldclimbcryptogateway_shibbep20_total . '&from=' . strtolower($shieldclimbcryptogateway_shibbep20_currency), array('timeout' => 30));
...
if ($shieldclimbcryptogateway_shibbep20_conversion_resp && isset($shieldclimbcryptogateway_shibbep20_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$shieldclimbcryptogateway_shibbep20_feesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/shib/aff-fees.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_shibbep20_revfeesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/shib/convert.php?value=' . $shieldclimbcryptogateway_shibbep20_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$shieldclimbcryptogateway_shibbep20_response_minimum = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/shib/aff-info.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_shibbep20_gen_wallet = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/shib/wallet.php?address=' . $this->shibbep20_wallet_address .'&callback=' . urlencode($shieldclimbcryptogateway_shibbep20_callback), array('timeout' => 30));
...
if (is_wp_error($shieldclimbcryptogateway_shibbep20_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($shieldclimbcryptogateway_shibbep20_wallet_decbody && isset($shieldclimbcryptogateway_shibbep20_wallet_decbody['address_in'])) {
		...
		$shieldclimbcryptogateway_shibbep20_genqrcode_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/shib/qrcode.php?address=' . $shieldclimbcryptogateway_shibbep20_gen_addressIn, array('timeout' => 30));
...
function shieldclimbcryptogateway_add_instant_payment_gateway_shibbep20($gateways) {
    $gateways[] = 'shieldclimb_Crypto_Payment_Gateway_Shibbep20';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbcryptogateway_add_instant_payment_gateway_shibbep20');
}

// Add custom endpoint for reading crypto payment status

   function shieldclimbcryptogateway_shibbep20_check_order_status_rest_endpoint() {
        register_rest_route('shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-check-order-status-shibbep20/', array(
            'methods'  => 'GET',
    ...
    function shieldclimbcryptogateway_shibbep20_check_order_status_callback($request) {
		...
		$shieldclimbcryptogateway_shibbep20_live_status_nonce = sanitize_text_field($request->get_param('nonce'));
        ...
        if ( empty( $shieldclimbcryptogateway_shibbep20_live_status_nonce ) || $order->get_meta('shieldclimb_shibbep20_status_nonce', true) !== $shieldclimbcryptogateway_shibbep20_live_status_nonce ) {
...
function shieldclimbcryptogateway_shibbep20_change_order_status_rest_endpoint() {
    ...
    register_rest_route( 'shieldclimbcryptogateway/v1', '/shieldclimbcryptogateway-shibbep20/', array(
        'methods'  => 'GET',
...
function shieldclimbcryptogateway_shibbep20_change_order_status_callback( $request ) {
    $order_id = absint($request->get_param( 'order_id' ));
	$shieldclimbcryptogateway_shibbep20getnonce = sanitize_text_field($request->get_param( 'nonce' ));
	$shieldclimbcryptogateway_shibbep20paid_value_coin = sanitize_text_field($request->get_param('value_coin'));
	$shieldclimbcryptogateway_shibbep20_paid_coin_name = sanitize_text_field($request->get_param('coin'));
	$shieldclimbcryptogateway_shibbep20_paid_txid_in = sanitize_text_field($request->get_param('txid_in'));
	...
	// Verify nonce
    if ( empty( $shieldclimbcryptogateway_shibbep20getnonce ) || $order->get_meta('shieldclimb_shibbep20_nonce', true) !== $shieldclimbcryptogateway_shibbep20getnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-crypto-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-shibbep20'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-shibbep20' === $order->get_payment_method() ) {
		
		// Get the expected amount and coin
	$shieldclimbcryptogateway_shibbep20expected_amount = $order->get_meta('shieldclimb_shibbep20_payin_amount', true) * $order->get_meta('shieldclimb_shibbep20_tolerance_percentage', true);

	
		if ( $shieldclimbcryptogateway_shibbep20paid_value_coin < $shieldclimbcryptogateway_shibbep20expected_amount || $shieldclimbcryptogateway_shibbep20_paid_coin_name !== 'bep20_shib') {
			// Mark the order as failed and add an order note
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */			
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s bep20_shib. TXID: %4$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_shibbep20paid_value_coin, $shieldclimbcryptogateway_shibbep20_paid_coin_name, $shieldclimbcryptogateway_shibbep20expected_amount, $shieldclimbcryptogateway_shibbep20_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
...
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_shibbep20paid_value_coin, $shieldclimbcryptogateway_shibbep20_paid_coin_name, $shieldclimbcryptogateway_shibbep20_paid_txid_in));
