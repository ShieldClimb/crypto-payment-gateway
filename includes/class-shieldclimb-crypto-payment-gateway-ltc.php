...
add_action('rest_api_init', function() {
    ...
    register_rest_route('shieldclimbcryptogateway/v1', '/check-order-status-ltc/', array(
        'methods'  => 'GET',
    ...
    register_rest_route('shieldclimbcryptogateway/v1', '/update-order-status-ltc/', array(
        'methods'  => 'GET',
...
function shieldclimbcryptogateway_check_status_permission($request) {
    ...
    // Nonce validation
    return $order->get_meta('shieldclimb_ltc_status_nonce', true) === $nonce;
}
...
function shieldclimbcryptogateway_update_status_permission($request) {
    ...
    // Nonce validation
    return $order->get_meta('shieldclimb_ltc_nonce', true) === $nonce;
}
...
function shieldclimbcryptogateway_update_order_status_callback($request) {
    ...
    if ('shieldclimb-crypto-payment-gateway-ltc' !== $order->get_payment_method()) {
        return new WP_Error(
    ...
    $tolerance = (float)$order->get_meta('shieldclimb_ltc_tolerance_percentage', true);
    $minimum_required = $expected_amount * $tolerance;

    // Payment validation
    if ($coin_type !== 'ltc' || $paid_value < $minimum_required) {
