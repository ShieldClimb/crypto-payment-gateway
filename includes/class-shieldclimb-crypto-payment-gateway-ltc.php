...
function shieldclimbcryptogateway_ltc_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'shieldclimb-crypto-payment-gateway-ltc'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-crypto-payment-gateway-ltc' === $order->get_payment_method() ) {
