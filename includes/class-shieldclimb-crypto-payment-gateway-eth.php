...
protected $icon_url;
    ...
    public function __construct() {
        ...
        $this->method_title       = esc_html__('ShieldClimb – Crypto Payment Gateway (Ethereum erc20 | Min 0.007515eth | Auto-hide if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
...
function shieldclimbcryptogateway_eth_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'shieldclimb-crypto-payment-gateway-eth'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-crypto-payment-gateway-eth' === $order->get_payment_method() ) {
