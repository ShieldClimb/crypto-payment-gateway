...
add_action('plugins_loaded', 'init_shieldclimbcryptogateway_daibep20_gateway');

function init_shieldclimbcryptogateway_daibep20_gateway() {
...
protected $icon_url;
    ...
    public function __construct() {
        $this->id                 = 'shieldclimb-daibep20';
        $this->icon = esc_url(plugin_dir_url(__DIR__) . 'static/daibep20.png');
        $this->method_title       = esc_html__('Dai Token bep20  - ShieldClimb Crypto Payment Gateway (Auto-hide at checkout if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
        $this->method_description = esc_html__('Dai Token bep20 crypto payment gateway with instant payouts, no sign-up, no KYC, and on-site checkout, sending funds directly to your bep20_dai wallet.', 'shieldclimb-crypto-payment-gateway'); // Escaping description
...
$shieldclimbcryptogateway_daibep20_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/dai/convert.php?value=' . $shieldclimbcryptogateway_daibep20_total . '&from=' . strtolower($shieldclimbcryptogateway_daibep20_currency), array('timeout' => 30));
...
if ($shieldclimbcryptogateway_daibep20_conversion_resp && isset($shieldclimbcryptogateway_daibep20_conversion_resp['value_coin'])) {
    ...
    return null;
		...
		$shieldclimbcryptogateway_daibep20_feesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/dai/aff-fees.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_daibep20_revfeesest_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/dai/convert.php?value=' . $shieldclimbcryptogateway_daibep20_feesest_reference_total . '&from=usd', array('timeout' => 30));
...
$shieldclimbcryptogateway_daibep20_response_minimum = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/dai/aff-info.php', array('timeout' => 30));
...
$shieldclimbcryptogateway_daibep20_gen_wallet = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/dai/wallet.php?address=' . $this->daibep20_wallet_address .'&callback=' . urlencode($shieldclimbcryptogateway_daibep20_callback), array('timeout' => 30));
...
if (is_wp_error($shieldclimbcryptogateway_daibep20_gen_wallet)) {
 ...
 // Check if decoding was successful
    if ($shieldclimbcryptogateway_daibep20_wallet_decbody && isset($shieldclimbcryptogateway_daibep20_wallet_decbody['address_in'])) {
		...
		$shieldclimbcryptogateway_daibep20_genqrcode_response = wp_remote_get('https://api.shieldclimb.com/crypto/bep20/dai/qrcode.php?address=' . $shieldclimbcryptogateway_daibep20_gen_addressIn, array('timeout' => 30));
...
function shieldclimbcryptogateway_add_instant_payment_gateway_daibep20($gateways) {
    $gateways[] = 'shieldclimb_Crypto_Payment_Gateway_Daibep20';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbcryptogateway_add_instant_payment_gateway_daibep20');
}
...
// Add custom endpoint for reading crypto payment status
   ...
   function shieldclimbcryptogateway_daibep20_check_order_status_rest_endpoint() {
    ...
    function shieldclimbcryptogateway_daibep20_check_order_status_callback($request) {
		...
		$shieldclimbcryptogateway_daibep20_live_status_nonce = sanitize_text_field($request->get_param('nonce'));
        ...
        if ( empty( $shieldclimbcryptogateway_daibep20_live_status_nonce ) || $order->get_meta('shieldclimb_daibep20_status_nonce', true) !== $shieldclimbcryptogateway_daibep20_live_status_nonce ) {
...
function shieldclimbcryptogateway_daibep20_change_order_status_callback( $request ) {
    $order_id = absint($request->get_param( 'order_id' ));
	$shieldclimbcryptogateway_daibep20getnonce = sanitize_text_field($request->get_param( 'nonce' ));
	$shieldclimbcryptogateway_daibep20paid_value_coin = sanitize_text_field($request->get_param('value_coin'));
	$shieldclimbcryptogateway_daibep20_paid_coin_name = sanitize_text_field($request->get_param('coin'));
	$shieldclimbcryptogateway_daibep20_paid_txid_in = sanitize_text_field($request->get_param('txid_in'));
	...
	// Verify nonce
    if ( empty( $shieldclimbcryptogateway_daibep20getnonce ) || $order->get_meta('shieldclimb_daibep20_nonce', true) !== $shieldclimbcryptogateway_daibep20getnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-crypto-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-daibep20'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-daibep20' === $order->get_payment_method() ) {
		
		// Get the expected amount and coin
	$shieldclimbcryptogateway_daibep20expected_amount = $order->get_meta('shieldclimb_daibep20_payin_amount', true) * $order->get_meta('shieldclimb_daibep20_tolerance_percentage', true);

	
		if ( $shieldclimbcryptogateway_daibep20paid_value_coin < $shieldclimbcryptogateway_daibep20expected_amount || $shieldclimbcryptogateway_daibep20_paid_coin_name !== 'bep20_dai') {
			// Mark the order as failed and add an order note
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */			
$order->update_status('failed', sprintf(__( '[Order Failed] Customer sent %1$s %2$s instead of %3$s bep20_dai. TXID: %4$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_daibep20paid_value_coin, $shieldclimbcryptogateway_daibep20_paid_coin_name, $shieldclimbcryptogateway_daibep20expected_amount, $shieldclimbcryptogateway_daibep20_paid_txid_in));
/* translators: 1: Paid value in coin, 2: Paid coin name, 3: Expected amount, 4: Transaction ID */
...
$order->add_order_note(sprintf( __( '[Payment completed] Customer sent %1$s %2$s TXID:%3$s', 'shieldclimb-crypto-payment-gateway' ), $shieldclimbcryptogateway_daibep20paid_value_coin, $shieldclimbcryptogateway_daibep20_paid_coin_name, $shieldclimbcryptogateway_daibep20_paid_txid_in));
