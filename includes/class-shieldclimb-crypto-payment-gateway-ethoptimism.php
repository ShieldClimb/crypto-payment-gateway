...
function shieldclimbcryptogateway_ethoptimism_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'shieldclimb-crypto-payment-gateway-ethoptimism'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-crypto-payment-gateway-ethoptimism' === $order->get_payment_method() ) {
