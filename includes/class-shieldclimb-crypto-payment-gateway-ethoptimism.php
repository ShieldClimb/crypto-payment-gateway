...
function shieldclimbcryptogateway_ethoptimism_check_order_status_rest_endpoint() {
    register_rest_route(
        ...
        '/shieldclimbcryptogateway-check-order-status-ethoptimism/', 
        array(
            'methods'  => 'GET',
            'callback' => 'shieldclimbcryptogateway_ethoptimism_check_order_status_callback',
            'permission_callback' => 'shieldclimbcryptogateway_ethoptimism_check_status_permission'
        )
    );
}
...
function shieldclimbcryptogateway_ethoptimism_check_status_permission($request) {
    ...
    if ($order->get_meta('shieldclimb_ethoptimism_status_nonce', true) !== $nonce) {
        return new WP_Error(
            'rest_forbidden',
...
function shieldclimbcryptogateway_ethoptimism_change_order_status_rest_endpoint() {
    register_rest_route(
        'shieldclimbcryptogateway/v1',
        '/shieldclimbcryptogateway-ethoptimism/',
        array(
            'methods'  => 'GET',
            'callback' => 'shieldclimbcryptogateway_ethoptimism_change_order_status_callback',
            'permission_callback' => 'shieldclimbcryptogateway_ethoptimism_change_status_permission'
        )
    );
}
...
function shieldclimbcryptogateway_ethoptimism_change_status_permission($request) {
    $order_id = absint($request->get_param('order_id'));
    $nonce = sanitize_text_field($request->get_param('nonce'));
    ...
    if ($order->get_meta('shieldclimb_ethoptimism_nonce', true) !== $nonce) {
        return new WP_Error(
            'rest_forbidden',
            __('Invalid nonce.', 'shieldclimb-crypto-payment-gateway'),
            array('status' => 403)
...
function shieldclimbcryptogateway_ethoptimism_change_order_status_callback($request) {
    ...
    if (!in_array($order->get_status(), ['processing', 'completed'], true) 
        && 'shieldclimb-crypto-payment-gateway-ethoptimism' === $order->get_payment_method()
    ) {
