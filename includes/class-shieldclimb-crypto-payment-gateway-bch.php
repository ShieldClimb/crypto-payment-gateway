...
add_action('rest_api_init', function() {
    ...
    register_rest_route('shieldclimbcryptogateway/v1', '/check-order-status-bch/', array(
        'methods'  => 'GET',
    ...
    register_rest_route('shieldclimbcryptogateway/v1', '/update-order-status-bch/', array(
        'methods'  => 'GET',
...
function shieldclimbcryptogateway_check_status_permission($request) {
    ...
    // Nonce validation
    return $order->get_meta('shieldclimb_bch_status_nonce', true) === $nonce;
}
...
function shieldclimbcryptogateway_update_status_permission($request) {
    ...
    // Nonce validation
    return $order->get_meta('shieldclimb_bch_nonce', true) === $nonce;
}
...
function shieldclimbcryptogateway_update_order_status_callback($request) {
    ...
    if ('shieldclimb-crypto-payment-gateway-bch' !== $order->get_payment_method()) {
        return new WP_Error(
    ...
    $tolerance = (float)$order->get_meta('shieldclimb_bch_tolerance_percentage', true);
    $minimum_required = $expected_amount * $tolerance;

    // Payment validation
    if ($coin_type !== 'bch' || $paid_value < $minimum_required) {
