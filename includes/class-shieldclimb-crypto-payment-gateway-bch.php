...
protected $icon_url;
    ...
    public function __construct() {
        ...
        $this->method_title       = esc_html__('ShieldClimb – Crypto Payment Gateway (Bitcoin Cash | Min 0.000835bch | Auto-hide if below min)', 'shieldclimb-crypto-payment-gateway'); // Escaping title
...
function shieldclimbcryptogateway_bch_change_order_status_callback( $request ) {
    ...
    // Check if the order is pending and payment method is 'shieldclimb-crypto-payment-gateway-bch'
    if ( $order && !in_array($order->get_status(), ['processing', 'completed'], true) && 'shieldclimb-crypto-payment-gateway-bch' === $order->get_payment_method() ) {
